...
- **[HEADER FIXES]** হেডারের সমস্যাগুলো সম্পূর্ণ ফিক্স করা হয়েছে:
  ...
  6. **[FULL EN LOCALIZATION]** `notice.php`, `projects.php` এবং `index.php` তে স্মার্ট দ্বিভাষিক হেল্পার যোগ করা হয়েছে যাতে `EN` মোডে কোনো নোটিশ বা প্রজেক্ট বাংলায় না থাকে এবং ১০০% খাঁটি ইংরেজিতে প্রদর্শিত হয়।
- **[ADMIN AUDIT & FIXES]** এডমিন প্যানেলের সমস্যাগুলো অডিট ও সমাধান করা হয়েছে:
  1. **Session & Cookie Resilience:** সার্ভারের `logs/sessions` পারমিশন ইস্যু দূর করতে ফলব্যাক সেফ পাথ ও `samesite => Lax` যুক্ত করা হয়েছে, যাতে লগইনের পর সেশন ড্রপ বা রিডাইরেক্ট লুপ না হয়।
  2. **Rate Limiting Lock Loop Fix:** ভুল পাসওয়ার্ড দিলে রেট লিমিট ট্র্যাকিংয়ে যাতে অনির্দিষ্টকালের জন্য লক না হয়ে যায় তা ফিক্স করা হয়েছে এবং সফল লগইনে পুরানো ফেইল্ড অ্যাটেম্পট ক্লিয়ার করা হয়েছে।
  3. **Schema & Dynamic Column Sync:** `schema.sql` এ missing টেবিলগুলো যোগ করা হয়েছে এবং `blogs_admin.php` তে `title_bn`/`title` ও `content_bn`/`content` এর জন্য ডাইনামিক হ্যান্ডলিং নিশ্চিত করা হয়েছে।
